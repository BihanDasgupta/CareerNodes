...
st.subheader("❤︎ AI Internship Graph Matcher with Rerank Optimization")

# Form Inputs
gpa = st.number_input("GPA", min_value=0.0, max_value=4.0, step=0.01, format="%0.2f", value=None)
if gpa == 0.0:
    gpa = None
education = st.selectbox("Education Level", ["Choose an option","High School Junior", "High School Senior", "High School Diploma",
    "Undergrad Freshman", "Undergrad Sophomore", "Undergrad Junior", "Undergrad Senior", "Bachelor's Degree", "Associates Degree", "Grad Student"])
...
if st.button("Find Matches"):
    ...
    sorted_results = sorted(zip(rerank_response.results, internships), key=lambda x: x[0].relevance_score, reverse=True)
    
    st.subheader("\u2315 Top Matches:")
    for result, internship in sorted_results:
    ...
    G = nx.Graph()
    G.add_node("You")
    for result, internship in sorted_results[:50]:
        node = f"{internship['company']}\n{internship['title']}"
        G.add_node(node)
        G.add_edge("You", node, weight=result.relevance_score)
